...
def generate_stream_chatglm3(model: PreTrainedModel, tokenizer: PreTrainedTokenizer, params: dict):
    ...
    predefined = params.get("predefined", None)

    if predefined is not None:
        assert predefined[0] == "\n", "add \\n before your strings because it affects tokenization"
        predefined_tokens = tokenizer(predefined)["input_ids"][4:] + [tokenizer.eos_token_id]
        predefined_tokens = torch.tensor([predefined_tokens]).to(model.device)
    ...
    for total_ids in model.stream_generate(**inputs, eos_token_id=eos_token_id, **gen_kwargs):
        if logprobs:
            total_ids, next_token_logprobs = total_ids[0], total_ids[1]
        total_ids = total_ids.tolist()[0]
        if logprobs:
            next_token_logprobs = next_token_logprobs[0][total_ids[-1]].item()
...
def generate_chatglm3(model: PreTrainedModel, tokenizer: PreTrainedTokenizer, params: dict):
    if params["messages"][-1].name is not None:
        params['logprobs'] = True
        params["predefined"] = "\n" + params["messages"][-1].name
        next_token_logprobs = []
        for response in generate_stream_chatglm3(model, tokenizer, params):
            next_token_logprobs.append(response['usage']['next_token_logprobs'])
            pass
        response = (response, next_token_logprobs)
    else:
        for response in generate_stream_chatglm3(model, tokenizer, params):
            pass
    return response
