...
class UsageInfo(BaseModel):
    ...
    next_token_logprobs: Optional[float] = 0.0
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if isinstance(response, tuple):
        response, next_token_logprobs = response[0], response[1]
        next_token_logprobs = json.dumps(next_token_logprobs)
    ...
    message = ChatMessage(
        role="assistant",
        content=response["text"] if next_token_logprobs is None else
        response["text"] + ",next_token_logprobs:" + next_token_logprobs,
